...
public struct RunSession: Codable, Identifiable, Equatable {
    ...
    let route: [LocationCoordinate2D]
    let paceData: [PacePoint]
    let type: RunType
    let participants: [UUID]?
    
    init(id: UUID = UUID(), userId: UUID, startTime: Date = Date(), endTime: Date? = nil, 
         distance: Double = 0, averagePace: Double = 0, route: [LocationCoordinate2D] = [], 
    ...
    private func validateRoute() throws {
        for coordinate in route {
            guard LocationCoordinate2DIsValid(coordinate) else {
...
// MARK: - Codable Implementation for LocationCoordinate2D Array
extension RunSession {
    ...
    init(from decoder: Decoder) throws {
        ...
        var decodedRoute: [LocationCoordinate2D] = []
        ...
        while !routeContainer.isAtEnd {
            ...
            decodedRoute.append(LocationCoordinate2D(latitude: latitude, longitude: longitude))
        }
        route = decodedRoute
    }
