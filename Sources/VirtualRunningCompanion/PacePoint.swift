...
public struct PacePoint: Codable, Equatable {
    ...
    init(timestamp: Date = Date(), location: LocationCoordinate2D, pace: Double, heartRate: Int? = nil) {
    ...
    private func validateLocation() throws {
        guard LocationCoordinate2DIsValid(location) else {
...
extension PacePoint {
    ...
    init(from decoder: Decoder) throws {
        ...
        location = LocationCoordinate2D(latitude: latitude, longitude: longitude)
    }
